...
public class UserController {
    ...
    public User getById(int user_id) {
        ...
        rs = koneksi.executeSelect("SELECT * FROM users WHERE id = ?", data);
        if(rs != null) {
            try {
                while(rs.next()) {
                    Timestamp timestamp = rs.getTimestamp(5);
    ...
    public Boolean insert (User pengguna) {
        ...
        if(pengguna.getId() != 0) {
            if(koneksi.execute("INSERT INTO pengguna (id, name, username, password) " 
                    + "VALUES (?, ?, ?, ?)", data)) {
                return true;
            } else {
                return false;
            ...
            if(koneksi.execute("INSERT INTO users (name, username, password) " 
                    + "VALUES (?, ?, ?)", data)) {
                return true;
            } else {
                return false;
